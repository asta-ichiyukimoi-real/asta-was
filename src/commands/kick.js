...
module.exports = {
    ...
    onRun: async (sock, msg, args) => {
        ...
        try {
            ...
            console.log('Kick bot IDs:', {
                id: sock.user?.id,
                lid: sock.user?.lid,
                jid: sock.user?.jid
            });

            console.log('Kick matched bot participant:', botParticipant);
            ...
            const botParticipantJid = normalizeJid(
                botParticipant.id || botParticipant.jid || botParticipant.phoneNumber
            );

            const validTargets = targets.filter(
                target => normalizeJid(target) !== botParticipantJid
            );
            ...
            await sock.sendMessage(
                groupId,
                {
                    text: `Removed ${formatMentions(validTargets)}.`,
